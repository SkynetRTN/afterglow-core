...
def upgrade():
    with op.batch_alter_table(
            ...
            table_args=(
                    sa.CheckConstraint(
                        'token_type is null or length(token_type) <= 40'),
                    sa.CheckConstraint(
                        'client_id is null or length(client_id) <= 48'),
        ...
        batch_op.add_column(sa.Column(
            'access_token_revoked_at', sa.Integer, nullable=False, default=0))
        batch_op.add_column(sa.Column(
            'refresh_token_revoked_at', sa.Integer, nullable=False, default=0))
