...
def upgrade():
    op.create_table(
        ...
        sa.CheckConstraint('description is null or length(description) <= 255'),
    )
    ...
    op.create_table(
        ...
        sa.Column('first_name', sa.String(255), default=''),
        sa.Column('last_name', sa.String(255), default=''),
        ...
        sa.CheckConstraint('username is null or length(username) <= 255'),
        sa.CheckConstraint('email is null or length(email) <= 255'),
        sa.CheckConstraint('password is null or length(password) <= 255'),
        sa.CheckConstraint('first_name is null or length(first_name) <= 255'),
        sa.CheckConstraint('last_name is null or length(last_name) <= 255'),
        sa.CheckConstraint('settings is null or length(settings) <= 1048576'),
    )
    ...
    op.create_table(
        ...
        sa.Column('client_id', sa.String(40), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column('auth_method', sa.String(40), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', sa.Integer(), primary_key=True),
        sa.Column('user_id', sa.Integer(), sa.ForeignKey('users.id'), nullable=False),
        sa.Column('token_type', sa.String(40), default='personal'),
    ...
    op.create_table(
        ...
        sa.Column('auth_time', sa.Integer(), nullable=False, default=lambda: int(time.time())),
        ...
        sa.CheckConstraint('code_challenge_method is null or length(code_challenge_method) <= 48'),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            'user_id', sa.Integer(),
            sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('token_type', sa.String(40), default='oauth2'),
        sa.Column('note', sa.Text(), default=''),
        sa.Column('client_id', sa.String(48)),
        sa.Column('access_token', sa.String(255), unique=True, nullable=False),
        ...
        sa.Column('issued_at', sa.Integer(), nullable=False, default=lambda: int(time.time())),
