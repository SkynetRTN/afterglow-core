...
class StackingJob(Job):
    ...
    def run(self):
        ...
        if settings.mode not in ('average', 'sum', 'median', 'percentile', 'mode'):
            raise ValueError('Stacking mode must be "average", "sum", "median", "percentile", or "mode"')

        if settings.mode == 'percentile' and not 0 <= settings.percentile <= 100:
        ...
        if settings.scaling is not None:
            ...
            if settings.scaling not in ('none', 'average', 'median', 'mode', 'histogram'):
                raise ValueError('Scaling mode must be "none", "average", "median", "mode", or "histogram"')
        ...
        if settings.prescaling is not None:
            ...
            if settings.prescaling not in ('none', 'average', 'median', 'mode', 'histogram'):
                raise ValueError('Prescaling mode must be "none", "average", "median", "mode", or "histogram"')
        ...
        if settings.rejection is not None:
            ...
            if settings.rejection not in ('none', 'chauvenet', 'iraf', 'minmax', 'sigclip', 'rcr'):
                raise ValueError('Rejection mode must be "none", "chauvenet", "rcr", "iraf", "minmax", or "sigclip"')
        ...
        if settings.rejection == 'iraf':
            if lo is not None:
                if lo % 1:
                    raise ValueError('Number of lowest values to clip for rejection=iraf must be integer')
                lo = int(lo)
            if hi is not None:
                if hi % 1:
                    raise ValueError('Number of highest values to clip for rejection=iraf must be integer')
                hi = int(hi)
        elif settings.rejection in ('chauvenet', 'rcr'):
            if lo is not None:
                if lo != 0 and lo != 1:
                    raise ValueError('Negative clipping flag for rejection=chauvenet|rcr must be 0 or 1')
                lo = bool(int(lo))
            if hi is not None:
                if hi != 0 and hi != 1:
        ...
        try:
            ...
            shape = data_files[0][0].header['NAXIS1'], data_files[0][0].header['NAXIS2']
            for i, data_file in enumerate(list(data_files[1:])):
                if (data_file[0].header['NAXIS1'], data_file[0].header['NAXIS2']) != shape:
            ...
            data, header = combine(
                ...
                prescaling=settings.prescaling,
        ...
        try:
            self.result.file_id = create_data_file(
                self.user_id, None, get_root(self.user_id), data, header, duplicates='append',
