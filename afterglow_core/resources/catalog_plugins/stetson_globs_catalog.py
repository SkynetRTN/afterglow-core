...
class StetsonGlobsCatalog(VizierCatalog):
    ...
    col_mapping = {
        'id': 'Star',
        'ra_hours': 'int(RAJ2000.value[0].split()[0]) + '
        'int(RAJ2000.value[0].split()[1])/60 + '
        'float(RAJ2000.value[0].split()[2])/3600',
        'dec_degs': '(abs(int(DEJ2000.value[0].split()[0])) + '
        'int(DEJ2000.value[0].split()[1])/60 + '
        'float(DEJ2000.value[0].split()[2])/3600)*'
        '(1 - 2*(DEJ2000.value[0].split()[0].startswith("-")))',
    }
    mags = {
